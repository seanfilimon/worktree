...
- **tokio-tungstenite WebSocket Router Bug:** Fixed a `501 Not Implemented` handshake error. Changed `mux.Handle("GET /staged/ws", ...)` to `mux.Handle("/staged/ws", ...)` in the Go `router.go`, allowing `websocket.Accept()` to successfully capture the protocol hijack instead of failing strict method matching. Fixed the Go status recorder middleware to properly implement `http.Hijacker`.
- **Metadata-Only Snapshot Constraint:** Fixed a `422 Unprocessable Entity` infinite background retry loop. Removed the `len(r.Objects) > 0` validation constraint in the Go server because W0rktree allows purely metadata-driven branch changes or config updates that have 0 physical file blobs.
- **CLI Staged Timestamp Display:** Fixed `"unknown time"` in `wt staged --watch`. Updated the CLI JSON parsing logic to check for the `created_at` field returned by the Go server's `Snapshot` model instead of purely `timestamp`.

## Current Limitations & Debt

### Staged Visibility Payload Granularity
The `wt staged` CLI command successfully displays incoming team snapshots (author, branch, tree, message, timestamp, and total file count). However, it does not detail *what kind* of activity each file represents (e.g., additions vs deletions).
**Why:** The Go Server `Snapshot` payload currently only broadcasts an array of `ObjectRef` (path, size, hash). It does not transmit the 3-way delta state (added/modified/deleted) because the server doesn't execute diffs on ingestion.
**Future Fix:** The local Rust Daemon must compute the `SnapshotChanges` delta matrix and embed a high-level summary (e.g., `{"added": 1, "deleted": 0, "modified": 2}`) into the `POST /staged` payload. The Go server can then forward this lightweight JSON property over the WebSocket, allowing the CLI to display `- 1 deletion, + 2 additions` next to the snapshot.
